...
and utilities to enhance \textbf{roxygen}}
...
\hspace*{\fill}\\
\hlstd{}\ttfamily\noindent
\hlprompt{\usebox{\hlnormalsizeboxgreaterthan}{\ }}\hlfunctioncall{args}\hlkeyword{(}\hlsymbol{roxygen\usebox{\hlnormalsizeboxunderscore}and\usebox{\hlnormalsizeboxunderscore}build}\hlkeyword{)}\mbox{}
\normalfont
\hspace*{\fill}\\
\hlstd{}\begin{Schunk}
\begin{Soutput}
...
the roxygenized package; in fact, we can also use Rd2 in roxygen and
specify the tag \texttt{@nord} (no Rd) to suppress Rd creation. Another
problem is, we often forget to escape the percent symbol \texttt{\%}
...
Yet another feature to mention about \emph{rab()} is that it has an
option to {}``reformat'' the code in the usage and example sections;
this is due to the fact that roxygen will remove all the leading spaces
and indent in the R code, which makes it difficult to read especially
when the code is long. If we specify \texttt{reformat = TRUE} in \emph{rab()},
...
\hspace*{\fill}\\
\hlstd{}\ttfamily\noindent
\hlprompt{{\ }}{\ }{\ }\hlcomment{\usebox{\hlnormalsizeboxhash}\usebox{\hlnormalsizeboxhash}{\ }the{\ }reformatted{\ }code}\mbox{}
\normalfont
\hspace*{\fill}\\
\hlstd{}\ttfamily\noindent
\hlprompt{{\ }}\hlsymbol{roxygen\usebox{\hlnormalsizeboxunderscore}and\usebox{\hlnormalsizeboxunderscore}build}{\ }\hlassignement{=}{\ }\hlkeyword{function}\hlkeyword{(}\hlformalargs{pkg}\hlkeyword{,}{\ }\hlformalargs{roxygen.dir}{\ }\hleqformalargs{=}{\ }NULL\hlkeyword{,}{\ }\hlformalargs{install}{\ }\hleqformalargs{=}{\ }\hlnumber{FALSE}\hlkeyword{,}\hspace*{\fill}\\
\hlstd{}\hlprompt{{\ }}{\ }{\ }{\ }{\ }\hlformalargs{check}{\ }\hleqformalargs{=}{\ }\hlnumber{FALSE}\hlkeyword{,}{\ }\hlformalargs{check.opts}{\ }\hleqformalargs{=}{\ }\hlstring{""}\hlkeyword{,}{\ }\hlformalargs{remove.check}{\ }\hleqformalargs{=}{\ }\hlnumber{TRUE}\hlkeyword{,}{\ }\hlformalargs{escape}{\ }\hleqformalargs{=}{\ }\hlnumber{TRUE}\hlkeyword{,}\hspace*{\fill}\\
\hlstd{}\hlprompt{{\ }}{\ }{\ }{\ }{\ }\hlformalargs{reformat}{\ }\hleqformalargs{=}{\ }\hlnumber{TRUE}\hlkeyword{,}{\ }\hlformalargs{...}\hlkeyword{)}{\ }\hlkeyword{\usebox{\hlnormalsizeboxopenbrace}}\hspace*{\fill}\\
...
\end{par}
Note this functionality depends on the package \textbf{formatR} \citep{formatR},
and sometimes it might be not be appropriate to reformat the code,
e.g. the \texttt{\textbackslash{}dontrun\{\}} command in Rd can contain
arbitrary texts, which means there could be illegal R expressions
and \textbf{formatR} will be unable to format the code. In case of
errors, we can consider turning this feature off.
